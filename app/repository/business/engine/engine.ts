...
export class Engine {
      ...
      .exec(function(err, applets) {
        if (!err) {
          ...
          applets.forEach(function(applet) {
            ...
            logger.debug(
              `got: ${applet.name}, ${applet.lastRunDate}, ${
                applet.nextRunDate
              }`
            );
          }, this);
      ...
      .then(function(actions: IMongoServiceActionInstance[]) {
        ...
        const chain = actions.reduce(function(previous, item) {
          return previous.then(function(previousValue) {
            logger.info(
              `action '${item.serviceAction.name}' done, calling the next`
            );
  ...
  public static async handleAction(
    ...
    const d = Q.defer();
    logger.debug(
      `an action with '${action.serviceAction.service.name}' type is starting`
    );

    switch (action.serviceAction.service.key) {
      case "reddit":
        ActionRepository.handleRedditAction(action, previousAction).then(
          function(result) {
            logger.info(
              `handle action: ${
                action.serviceAction.name
              }, action is complete, resolving`
            );
            d.resolve(result);
          },
          function(reason) {
        ...
        break;
      case "slack":
        ActionRepository.handleSlackAction(action, previousAction).then(
          function(result) {
            logger.info(
              `handle action: ${
                action.serviceAction.name
        ...
        break;
      case "wordpress":
        ActionRepository.handleWordpressAction(action, previousAction).then(
          function(result) {
            logger.info(
              `handle action: ${
                action.serviceAction.name
        ...
        break;
      case "stumbleupon":
        ActionRepository.handleStumbleuponAction(action, previousAction).then(
          function(result) {
            logger.info(
              `handle action: ${
                action.serviceAction.name
